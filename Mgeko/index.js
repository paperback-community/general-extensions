...
var source = (() => {
  ...
  var Parser2 = class {
    ...
    ontextentity(cp, endIndex) {
      var _a5, _b;
      this.endIndex = endIndex - 1;
      (_b = (_a5 = this.cbs).ontext) === null || _b === void 0 ? void 0 : _b.call(_a5, fromCodePoint3(cp));
      this.startIndex = endIndex;
    }
    /**
  ...
  function isAsciiAlphaNumeric5(code) {
    return code >= CharCodes5.UPPER_A && code <= CharCodes5.UPPER_Z || code >= CharCodes5.LOWER_A && code <= CharCodes5.LOWER_Z || isNumber4(code);
  }
  function isEntityInAttributeInvalidEnd4(code) {
    return code === CharCodes5.EQUALS || isAsciiAlphaNumeric5(code);
  }
  ...
  var EntityDecoder4 = class {
    ...
    write(input, offset) {
      switch (this.state) {
        case EntityDecoderState4.EntityStart: {
          if (input.charCodeAt(offset) === CharCodes5.NUM) {
    ...
    stateNumericStart(input, offset) {
      ...
      if ((input.charCodeAt(offset) | TO_LOWER_BIT4) === CharCodes5.LOWER_X) {
  ...
  function getDecoder4(decodeTree) {
    let returnValue = "";
    const decoder = new EntityDecoder4(decodeTree, (data2) => returnValue += fromCodePoint4(data2));
  ...
  function determineBranch4(decodeTree, current, nodeIndex, char) {
  ...
  function decodeHTML2(htmlString, mode = DecodingMode4.Legacy) {
    return htmlDecoder4(htmlString, mode);
  }
  ...
  init_buffer();
  function restoreDiff2(array) {
    for (let index2 = 1; index2 < array.length; index2++) {
      array[index2][0] += array[index2 - 1][0] + 1;
    }
    return array;
  }
  ...
  var parseMangaDetails = ($2, mangaId) => {
    ...
    secondaryTitles.push(
      decodeHTMLEntity($2("img", "div.fixed-img").attr("alt")?.trim() ?? "")
    );
