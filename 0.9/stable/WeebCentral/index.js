...
var source = (() => {
  ...
  function getFilterTagsBySection(section, tags) {
    ...
    return Object.entries(values).filter((x) => x[1] == "included").map((x) => x[0]);
  }
  ...
  function getRating(rating) {
    return rating === "Yes" ? import_types2.ContentRating.ADULT : import_types2.ContentRating.EVERYONE;
  }

  // src/WeebCentral/WeebCentralInterceptor.ts
  init_buffer();
  var import_types3 = __toESM(require_lib(), 1);
  var WeebCentralInterceptor = class extends import_types3.PaperbackInterceptor {
    async interceptRequest(request) {
      request.headers = {
        ...request.headers,
        referer: `${WC_DOMAIN}/`
  ...
  var parseIntoTagSection = ($2, id, title, selector) => {
    ...
    for (const element of elements) {
      ...
      const id2 = $2("input", element).last().attr("value") ?? "";
  ...
  var WeebCentralExtension = class {
    ...
    async getDiscoverSections() {
      return [
        ...
        { id: "genres", title: "Genres", type: import_types4.DiscoverSectionType.genres }
      ];
    }
        ...
        case "genres": {
          ...
          items = genres.map((genre) => ({
            ...
            searchQuery: {
              title: "",
              filters: [
                { id: "included_tag" /* Genres */, value: { [genre.id]: "included" } }
              ]
            },
            name: genre.title,
            metadata
    ...
    async getSearchResults(query, metadata) {
      ...
      newUrlBuilder = newUrlBuilder.addQuery(
        ...
        "included_status" /* SeriesStatus */,
        getFilterTagsBySection("included_status" /* SeriesStatus */, query.filters)
      ).addQuery(
        "included_type" /* SeriesType */,
