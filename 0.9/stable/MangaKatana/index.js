...
var source = (() => {
  ...
  var MangaKatanaExtension = class {
    ...
    async getNewMangaSectionItems(section, metadata) {
      ...
      const collectedIds = metadata?.collectedIds ?? [];
      const request = {
        url: new URLBuilder(DOMAIN_NAME).addPath("new-manga").addQuery("page", String(page)).build(),
        method: "GET"
      };
      const $2 = await this.fetchCheerio(request);
      const items = [];
    ...
    async getSearchResults(query, metadata) {
      ...
      if (query.title && query.title.length < 3) {
        ...
        if (this._lastValidQuery && this._lastValidQuery.startsWith(query.title) && this._lastValidResults.items.length > 0) {
          return this._lastValidResults;
        }
        return { items: [], metadata: void 0 };
      }
      const searchFilters = await this.getSearchFilters();
      const genreFilter = searchFilters.find((filter4) => filter4.id === "genres");
      const genres2 = query.filters?.find((filter4) => filter4.id === "genres")?.value;
      const genreEntries = genres2 ? Object.entries(genres2) : [];
      const typedGenreOptions = genreFilter && "options" in genreFilter ? genreFilter.options : [];
      ...
      if (genreOnlySearch) {
        ...
        genreEntries.forEach(([genreId, inclusion]) => {
          if (inclusion === "included") {
            ...
            if (genreOption) {
              const formattedGenre = genreOption.value.toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "");
        ...
        if (query.title) {
          searchUrl.addQuery("search", encodeURIComponent(query.title)).addQuery("search_by", "book_name").addQuery("page", String(page));
        } else {
      ...
      let response, data2;
      try {
        [response, data2] = await Application.scheduleRequest(request);
        this.checkCloudflareStatus(response.status);
        const html3 = Application.arrayBufferToUTF8String(data2);
        const $2 = load(html3);
        const results = this.processSearchResults($2, response.url, page);
        if (results.items.length === 0 && query.title && this._lastValidQuery && query.title.includes(this._lastValidQuery) && this._lastValidResults.items.length > 0) {
        ...
        if (query.title && this._lastValidQuery && query.title.includes(this._lastValidQuery) && this._lastValidResults.items.length > 0) {
          return this._lastValidResults;
        }
        return { items: [], metadata: void 0 };
      }
    ...
    processSearchResults($2, responseUrl, currentPage) {
      const finalUrl = new import_types2.URL(responseUrl);
      const searchResults = [];
      if (finalUrl.pathname.startsWith("/manga/")) {
        ...
        const latestChapter = $2(".update_time").first().text().trim();
        if (title && mangaId) {
          searchResults.push({
            mangaId,
            title,
      ...
      } else if ($2("#search_rs .item").length > 0) {
        $2("#search_rs .item").each((_, element) => {
          const unit = $2(element);
      ...
      let nextPage;
      const nextPageLink = $2("a.next.page-numbers").attr("href");
      if (nextPageLink) {
